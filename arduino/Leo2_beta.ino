...
//
// Leo2 is a slave on the i2c bus.
// Version 1:  the master duino (the original leo, Leo1) is sending rpm as a 
// floating point value in a struct, via EasyTransfer library.
//
// the intent was for Leo2 to receive rpm data from the primary leo, and to use
// it to scale and send throttle data.  this version worked just fine.  BUT...
// leo 1 is pretty busy with computation for rpm (averaging, floating point math).  and
// then it also has to manage the i2c bus which has a bit of overhead. so...
// in version 2 (current):  leo 2 will do the reading of reed switches and analog inputs
// such as brake, and (depending on mode) either send or not send HID data.
//
//
...
//
// Mode 0:  Leo1 reads roll off the chuck and passes it as "tilt" to use for steering.
// Mode 1:  Leo1 reads rotation off the AS5601 and passes it to Leo2;
//             Leo2 reads rpm and brake directly.
// originally had planned to use joystick buttons for ETS2 control, but instead I
// now plan to put all user button presses and keystroke sends on Leo 1.
// this is not quite as clean but easier to implement (don't have to pass every button
// press to Leo 2).  I can include a controls_osx.sii file, of course.

// Apr 29 2017:  compiled and running, correctly reading reed switches, pot, and mode switch
// rudimentary LED indicators are working.  Time to turn on the master and try bidirectional
// i2c data sharing.
//
...
//
int ctrlMode = 0;                       // chuck, or real bike steering?
// actually the differences are so large that I think the chuck version is a separate release.
// otherwise we will be ifdef'd to death.
...
int lastBrake = 0;
int brakeNoise = 13;                   // jitter in hall effect sensor analog value -- ouch!
...
int brakeReads[numBrakeReads];         // noise between averages (pathetic)
int brakeReadPtr = 0;
int brakeTot = 0;
int brakeAvg = 0;
...
boolean invertSpinDir = 1;            // you will need this for switching a portable stepper
                                      // from standing mode to sitting (recumbent) mode
// there is some indecision about whether ctrlMode and invertSpinDir should be local switches
// rather than passed by Leo1.  for now let's pass them as a tutorial exercise.
// as of beta4 (remodelled physical dashboard) Leo2 reads spindir.
...
// don't believe any of these assignments!
...
// read the reed switches to determine rpm
 ...
 // by about 30 counts;  we hide this filtering in the brakeRead function
